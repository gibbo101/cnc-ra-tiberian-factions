...
# JUGGER.SHP draws the walker high in its own canvas. Lowering the walk frames by this many
# canvas pixels stands them on the same ground line as the deployed frames and the deploy
# ladder (lowest body pixel 363 on the 448 canvas), so the selection box brackets the walker's
# feet, its health bar clears the hull, and the sprite does not shift when it packs up.
WALK_DROP = 87
...
def crisp(img, drop=0):
    ...
    inf.safe_paste(out, scaled, round(CANVAS / 2 - scaled.width / 2), round(CANVAS / 2 - scaled.height / 2) + drop)
    return out


def muzzle_tip(bundle, facing):
    """The barrel tips in canvas pixels: the drawn bundle's leading pixels along the aim
    (screen forward = (-sin, -cos/2) of the facing), averaged. Art and fire point stay in
    lockstep because both come from this one placement."""
    import numpy as np
    a = np.array(bundle.split()[3], dtype=np.int16)
    ys, xs = np.nonzero(a > 200)
    if len(xs) == 0:
        return (CANVAS / 2.0, CANVAS / 2.0)
    th = math.radians(facing * 11.25)
    fx, fy = -math.sin(th), -math.cos(th) * 0.5
    n = math.hypot(fx, fy)
    proj = (xs * fx + ys * fy) / n
    lead = proj >= np.percentile(proj, 97)
    return (float(xs[lead].mean()), float(ys[lead].mean()))
...
def walk_frames():
    ...
    for f in range(8):                      # out facing f (CCW from N) <- SHP block (8-f)%8 (CW)
        ...
        for s in range(15):
            i = src * 15 + s
            out.append(crisp(inf.with_shadow(frame("jugger", i), frame("jugger", 120 + i)), WALK_DROP))
    return out


def deployed_frames(render_dir="renders_djuggbar", pitch=5.0, tips=None):
...
def write_muzzle_table(tips):
    """redalert/tsjugg_muzzle.h: the barrel tips as leptons east/south of the unit centre,
    one row per CCW facing, the rest set then the aiming set. Canvas pixels convert at 4/3
    (ShapeSize 56 on a 448 canvas, as the Titan's table does)."""
    rows = []
    for tx, ty in tips:
        rows.append((round((tx - CANVAS / 2.0) * 4.0 / 3.0), round((ty - CANVAS / 2.0) * 4.0 / 3.0)))
    out = "// GENERATED by scripts/ts_pack_jugg.py -- do not hand-edit.\n"
    out += "// Juggernaut barrel tips (leptons from the unit centre), per CCW facing.\n"
    out += "// [0] = deployed at rest, [1] = deployed aiming at a target.\n"
    out += "static const short _tsjugg_muzzle[2][32][2] = {\n"
    for half in (0, 1):
        out += "    {\n"
        for dx, dy in rows[half * 32:half * 32 + 32]:
            out += f"        {{{dx}, {dy}}},\n"
        out += "    },\n"
    out += "};\n"
    path = os.path.join(HERE, "..", "redalert", "tsjugg_muzzle.h")
    open(os.path.abspath(path), "w").write(out)
    print("wrote redalert/tsjugg_muzzle.h (muzzle table)")
...
def main():
    ...
    for render_dir, _, pitch in BARREL_SETS:
        dep += deployed_frames(render_dir, pitch, tips)
