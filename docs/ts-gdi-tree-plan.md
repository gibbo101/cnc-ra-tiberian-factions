...
## ⭐⭐⭐ RESUME HERE — NEXT SESSION: **test the war-factory rebuild build `a6c9602e` (branch `wf-rebuild`)** — deployed to the desktop prefix (DLL 0457fa8a). Cast one exit each of Titan / Disruptor / APC / harvester and check the 08-29 list: (1) no unit vanishing east of the door on the way out (cause was the open-doorway front layer reaching x=772; band now runs to the canvas edge), (2) the one grey seam pixel (pad cleared under the building's foot), (3) spawn on the doorway row moved WEST to col 2.38 (`TSWEAP_SEAT_MOUTH` 610,282 in `tsweap_exit_seats.inc`), (4) Titan on its own seat further back (610,192 = row 0.75; head must stay under the roof, feet in the door). Signed off already this arc: 5x3 L-footprint + 14-cell ghost, refinery-parity size, Lanczos-hard art, sandwich (front = hangar minus the aperture, cut LEFT along Luke's yellow line `resources/custom-art/tsweap-front-cut-line.json`, TOP along the rolled shutter's slanted edge, RIGHT open), open-doorway front while unloading (body has the door painted SHUT; GAWEAP_1 opens it), shutter layer +200, TS 5-state unload, `Rail_To` SE exit to XYCELL(4,2) with facing derived from the line (no sort clamp), TS's own lamp corner (fill-above removed). Branch is OFF ts-units 66717e43 — rebase onto main (1d3d7f24) before merging. Convergence: main = 1d3d7f24 (tag trunk-2026-08-28-convergence); subterranean's next arc `emp-cannon` off main, Deck only.
