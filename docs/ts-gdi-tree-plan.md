...
## ⭐⭐⭐ RESUME HERE — NEXT SESSION: **WF rebuild round 4, branch `wf-rebuild`, UNCOMMITTED working tree (seats + shutter clip + packer), deployed to the desktop prefix (DLL `8c88b6ce`, TSWEAPDR `b9177aec`).** Luke played it: "lots of work to do" — screencasts of a Titan exit and an APC exit are promised next session; measure them against the geometry below before changing anything. **Facts established 2026-08-29 (afternoon):**
- ⚠ **`Coord` of the building = the ORIGIN CELL'S CENTRE, not the plot's NW corner.** Every seat in `tsweap_exit_seats.inc` is added to it, so seat (lx,ly) → canvas (192 + lx/2, 208 + ly/2) on the 896x672 canvas (plot = 128 px/cell from canvas (128,144)). The 08-28 "col 2.69 = shutter centre" derivation assumed the corner and put units 64 px SE of intended; that (not only the front band) was the "vanish east", and the 08-29 move to 610 overshot west (APC centre at canvas 497 vs jamb 449 = a third of the hull hidden, measured on the 15:21 screencast). Current: mouth seat (690,282) → canvas (537,349); Titan (690,-108) → (537,154), feet at y=305 under the shut shutter's bottom (313). Left jamb = `resources/custom-art/tsweap-front-cut-line.json` (canvas x per row: 409 @ y255 … 449 @ y374). SE sprite half-widths: APC 129, Titan 115, Disruptor 152, harvester 114 (meta crop of frame 12).
- **The "annoying grey pixel" = TS's own GAWEAP_D door leaf has a bottom-right TAB (source x>=113, y>=113) that rolls up last and floats detached in stages 5-7; the shutter sorts +200, so it drew over any hull crossing the right jamb (only visible when a unit is under it).** Fixed in the packer: `EXTRA_LAYER_CLIPS` erases the box then `keep_largest_component()` drops fringe pixels; all 18 DR frames are single-piece. The base layer's painted-shut door keeps the idle look. UNVERIFIED in play.
- Titan's rail to XYCELL(4,2) from the deeper seat is ~30° steeper than SE (facing follows the line); if Luke wants pure SE the exit cell or seat x must move together.
- A full `ts_pack_tree.py` run rewrites every ZIP's timestamps; compare zip member CRCs against HEAD and `git checkout` the content-identical ones (only TSWEAPDR changed). `cmake -E copy_directory` the resources into build/ BEFORE rsync — a blocked deploy block skipped the restage once this session and shipped a stale ZIP with the new DLL (caught by the md5 line; always md5 the changed ZIP too).
- Worktree deploys drop the gitignored crest atlas: copied `MT_COMMANDBAR_COMMON.TGA` into build/ and the prefix this session.
