...
**Status:** **CLOSED — MEG route abandoned (2026-05-29); the GOAL was then achieved WITHOUT it (2026-08-30).** Step 0 (static RE of `ClientG.exe`) resolved the MEG question: a mod texture MEG *is* read by the front-end, but only as a **whole-file SHADOW** (the mod's `Textures_SRGB.meg` *replaces* base entirely → **2.4 GB**). **No merge-per-file → a tiny atlas-only MEG does NOT work.** That verdict stands.
**2026-08-30 (desktop, live): the front-end DOES render loose `Data/ART/TEXTURES/SRGB/` overrides — both standalone DDS screens AND `MT_COMMANDBAR_COMMON.TGA` atlas pixels.** Proven by enabling Reilsss CnCinRA: its custom main menu (loose `RA_MAINMENUBG_*` / `UI_RA_MAINLOGO` DDS) rendered in the shell, and its custom faction-picker icons rendered in the skirmish lobby — verified to live at the `_03`–`_10` regions *inside its loose atlas TGA* (cropped at base `.MTD` coords; no FACTIONS.XML, no MEG, no EMC in that mod). The old PROVEN #4 ("front-end ignores loose overrides") is **FALSIFIED**; the 05-29 repaint test's failure cause is unknown (prime suspect: the edited atlas never actually reached the game — cf. `reference-data-only-changes-dont-restage`). **Custom front-end pixels — picker emblems included — ship as loose files.** No texture MEG is needed for anything. **Confirmed from OUR OWN mod 2026-08-31, on camera:** repainted `_04`/`_05` slot regions in our deployed loose atlas rendered in the lobby dropdown. Starting-position markers are separate 40x40 regions (`UI_MAPSELECT_FACTION_NN`: `_04`=(4742,4278), `_05`=(1413,4297)) — painted and VERIFIED on the minimap in-game same night (Luke: "worked fine"). Loose `.MTD` is IGNORED (probed): region geometry is launcher-owned; art must fit the stock boxes.
...
- **0 of 96** subscribed Workshop mods ship **any** `.meg`. The established texture-override surface is **loose files** (8,587 DDS / 351 TGA across the cache) — and per the 2026-08-30 result the front-end shell **honors** them (the shell-ignores claim was the falsified old #4).
- The one mod that changes front-end art — **Reilsss CnCinRA** — does it with a **wholesale loose 176 MB atlas TGA** (`184,582,588 B`, ≈ the full base atlas) plus ~140 standalone front-end DDS. **Correction (2026-07-11 spike):** Reilsss ships **no DLL and no MEG** — it is a pure data mod; its ccmod.json **"(Requiers EMC)"** refers only to its CCDATA `[New*]` sections needing EMC's **sim-side** DLL to parse them. EMC is a `RedAlert.dll` loaded by `InstanceServerG` — a *different* process from the `ClientG` front-end (CNC-only exports, no d3d/graphics imports) — so it **cannot** hook the front-end texture read. Reilsss's art reaches the shell as **loose files `ClientG` reads itself** — confirmed rendering live 2026-08-30 (main menu + lobby picker icons). See [[spike-launcher-process-model]] / `launcher-vs-dll-ownership.md`.
...
- **Allied + Soviet emblems are UNBLOCKED (2026-08-30):** repaint their `_NN` regions in the loose atlas we already ship. Same lever gives any future faction (TS GDI, TS Nod, …) a bespoke picker emblem.

The loose-file result unlocks everything this spike was after: custom main-menu art, custom faction logos, any front-end image — all Workshop-shippable, no EMC, no MEG. It is the *image* sibling of the `config-meg-mod-delivery` breakthrough.

**Concrete texture surfaces this fixes at once** (all are atlas regions the launcher picks by faction/country — NO CONFIG.MEG field repoints them, confirmed; repaint their pixels in the loose atlas instead):
...
4. **The front-end HONORS loose `Data/ART/TEXTURES/SRGB/*` overrides — atlas included (FALSIFIED-then-corrected).** The 05-29 test (repainted picker flags in a loose atlas, no change after full restart) is contradicted by direct observation 2026-08-30: Reilsss CnCinRA's loose atlas renders its custom `_03`–`_10` picker icons in the shell's skirmish lobby, and its standalone DDS render the main menu. The 05-29 failure cause was never found — most likely the edited atlas never reached the game (stale copy; cf. `reference-data-only-changes-dont-restage`). When repainting OUR atlas, md5-verify the deployed file before judging the result.
...
## Related docs
- `ui-atlas-modding.md` — loose atlas override mechanics (its old "in-game only" scope limit is superseded by the 2026-08-30 result above — the shell renders loose overrides too)
