...
8. **EMC route is not usable.** Reilsss's CnCinRA changes front-end textures, but via the EMC DLL + loose `.DDS`; EMC conflicts with our own DLL, so we can't adopt it.
