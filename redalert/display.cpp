...
{
    // Tiberian Factions: UNIT_TDGMCV is the GDI/Nod MCV and must be excluded from
    // band/select-all just like the RA UNIT_AMCV. The TD harvester (UNIT_TDHARV) is
    // already covered by IsToHarvest below.
    return (obj->What_Am_I() == RTTI_UNIT)
           && (((UnitClass*)obj)->Class->IsToHarvest || *((UnitClass*)obj) == UNIT_AMCV
               || *((UnitClass*)obj) == UNIT_TDGMCV);
}
