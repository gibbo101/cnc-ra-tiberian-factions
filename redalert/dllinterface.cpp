...
#if TF_DEV_BUILD // TF_AI_DIAG -- candidate registry + resolution forensics.
    {
        FILE* f = TF_AI_Diag_File();
        if (f != NULL) {
            fprintf(f,
            ...
            if (ambiguous && TF_LobbyCandN > 0) {
                ...
                fprintf(f, "  RESOLVE site=%d branch=%c applied=%s\n", TF_LobbyProbeTag, branch, rv);
            }
            fflush(f);
        }
    }
...
{
    int mask = 0;
    if (ownable & HOUSEF_ALLIES) {
        mask |= TF_FACTION_ALLIES;
    }
    if (ownable & HOUSEF_SOVIET) {
        mask |= TF_FACTION_SOVIET;
    }
    if (ownable & HOUSEF_GDI) {
        mask |= TF_FACTION_GDI;
    }
...
#if TF_DEV_BUILD
/*
** One-shot dump of every buildable type's faction mask, consumed by
** scripts/cameo_badge_build.py to decide which emblem combinations to bake.
** Ownership for the faction-split types lives in C++ constructor defaults
** rather than rules.ini, so reading it back out of the running engine is the
** only way to get it exactly right.
*/
static void TF_Dump_Faction_Masks(void)
{
    ...
    char path[512];
    snprintf(path, sizeof(path), "%s/Documents/CnCRemastered/tf_faction_masks.txt", h);
...
static TF_ProducerMasks TF_Compute_Producer_Masks(HouseClass const* house)
{
    TF_ProducerMasks masks;
...
static void TF_Apply_Cameo_Badge(char* asset_name, int entry_owner_mask, int held_mask)
{
    ...
    if (held_count >= 2) {
        badge = entry_owner_mask & held_mask;
    }
